...
| `_flood_fill(x, y, target)` | BFS flood fill on `coloring_image`; stops at black outlines and different-color regions |
...
| `_load_builtin(path)` | Loads a built-in asset, flattens alpha, runs the full pipeline |
